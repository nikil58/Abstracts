...
+ #### Избавление от динных (медленных) веток
Необходимо избегать заведомо более длинных решений, если есть более простое. Например, если нам нужно что-то сделать, пока нет ошибки, то необязательно узнавать в этом же участке кода, что именно за ошибка возникла.
**Отказаться от:**
```cpp
if (checkForErrorA())
 handleErrorA();
...
```
_P.S. Виртуальные функции необходимо использовать тогда, когда неизвестно какой тип данных оижадется_

+ #### Использование памяти
По возможности, нужно избегать использования слов new и delete, т.к. они за собой тянут множество строк кода. Вместо этого можно использовать уже расположенные в памяти объекты, а также переиспользование объектов, вместо удаления. Если все же необходимо удалить какой-то большой объект, то лучше это сделать где-то снаружи "критической секции", в другом потоке. Также не стоит забывать о фрагментации памяти.

  _P.S. Необходимо помнить, что некоторые объекты имеют встроенные конструкторы, что может привести к перегрузке кэша инструкций_
  
+ #### Исключения (на gcc, clang, msvc)
Не стоит бояться использовать исключения. Их наличие в коде не слишком замедяет программу, а если исключение выбросилось, то никакой речи о скорости и быть не может (Необходимо записать лог, дамп ядра, выключение). 

 _P.S. Не стоит использовать исключения для потока управления_
 
+ #### Уменьшение логических веток
Не стоит использовать конструкции if-else для ветвистого участка кода. Вместо этого можно воспользоваться двумя подходами:
...
По возможности избегайте многопоточности в "критических секциях" кода. Для этого есть ряд причин:
  * Синхрониация с помощью прерываний - дорого. 
  * Блокировка свободного участка кода может требовать блокировки на уровне железа. 
  * Сложный параллелизм. 

  Если избежать многопоточности невозможно, то нужно придерживаться следующих правил:
  * Общие данные должны быть минимальны
  * Используйте копии данных, а не сами данные для обмена
  * По возможности отказаться от синхронизации (может, синхронизации не необходима, или архитектура машина предотвращает прерывания чтения/записи)
  
+ #### Поиск данных
Мы привыкли связывать данные по id, по ключевым полям, чтобы все данные были нормализованны. Однако, если вы хотите достигнуть большей производительности, денормализованные данные можно использовать. В частности, на примерах становится понятно, как использовать денормализованные данные:
...
+ #### Атрибуты компилятора
  ...
  + ##### ((always_inline)) и ((noinline))
    Позволяет определить, что более предпочтительно: вставить или прыгнуть в определенную облость с ассемблеровский кодом. Например:
    ```cpp
      void get_error_code() { ... }
      int main(int argc, char**) {
       if (argc > 1)
...
+ #### Горячий кэш
Пожалуй, самый наилучший способ ускорения работы вашего приложения. Так бывает, что ваша программа (в частности, критическая секция) какое-то длительное время ничем не занимается: нет подходящего ценника, предложения и т.п. В таком случае кэш засоряется другими инструкциями и для выгрузаки/загрузки данных потребуется какое-то время. Для решения этой проблемы достаточно "обмануть" критическую секцию. "Сказать" ей, чтобы она работала. При этом будет сохранятся кэш данных и кэш инструкций. Необходимо и достаточно время от времени запускать код критической секции, даже если в этом не было необходимости. 

+ #### Строки
В общем говоря, нужно стараться избегать строк с использованием ключевого слова "string", потому что иногда не нужен весь интерфейс string, а также другие проблемы, связанные с аллоцированием и др. Самым лучшим решение будет использование std::string_view или другие API для работы со строками:
  ```cpp
...
```
_P.S. В стандарта C++17 и C++14 доступно под названием std::experimental::string_view_

+ #### Использование Switch и Enum
Вы, наверное,  помните, что компилятор очень хорош в операциях сравнения на равенство/неравенство нулю. Так вот это стоит учитывать при использовании Switch. Если есть кейс, в который вы чаще всего будете и должны попадать, то его стоит сделать первым:
  ```cpp
...
+ #### Математика
Стоит очень осторожно использовать различные математические функции в GlibC, потому что они могут работать странно медленно:
  ```cpp
